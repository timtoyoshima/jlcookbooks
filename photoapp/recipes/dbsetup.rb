execute "mysql-create-table" do
  ...
  not_if "/usr/bin/mysql -u#{node[:deploy][:myphotoapp][:database][:username]} -p#{node[:deploy][:myphotoapp][:database][:password]} #{node[:deploy][:myphotoapp][:database][:database]} -e'SHOW TABLES' | grep [:photoapp][:dbtable]"
  action :run
end
